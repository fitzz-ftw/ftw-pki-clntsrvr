...
def prog_client_server_csr(argv: list[str] | None = None, **kwargs) -> int:
    """
    Execute the combined client and server Certificate Signing Request (CSR) generation process.

    This function processes command-line arguments to generate private keys
    and the corresponding CSRs for both client and server identities.
    ...
    try:
        ...
        save_pem(priv, config.private_keys / args.private_key, is_private=True)
        # !SECTION - Save private Key
        # SECTION - Save CSR
        san_args={"ip_addresses": args.ip_addresses, 
                  "dns_names": args.host_names}
        client_pem = clntsrvr_csr.build(load_private_key_from_pem(
               pem_data=priv, 
        ...
        pki_pack.save(config.pki_path/ args.pki_name)
        # !SECTION - pki- Container
        # SECTION - Cleanup
        conf_file.unlink()
        # !SECTION - Cleanup
